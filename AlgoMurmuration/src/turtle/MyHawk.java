...
public class MyHawk extends Turtle implements Predator {
    ...
    private void steerAroundObstacles(List<Obstacle> obstacles) {
        ...
        if (Math.abs(totalTurn) > 1) turn((int) totalTurn); //adsdjust turning based on avoidance
    }
    ...
    //predator hunts closest boid
    public void hunt(List<Prey> prey, int deltaTime) {
        //find closest prey (boid)
        double closestDistance = Double.MAX_VALUE;
        Prey closestPrey = null;

        //loop through the prey (boids) to find the closest one
        for (Prey p : prey) {
        ...
        //if a prey (boid) is found close enough, "eat" it
        if (closestPrey != null) {
            prey.remove(closestPrey);  //remove the boid from the list
    ...
    public boolean collisionCheck(SimulationObject other) {
        if (other instanceof MyBoid) {
            double distance = currentPosition.toroidDistance(other.getPosition(), canvasWidth, canvasHeight);
            if (distance < getCollisionRadius() + other.getCollisionRadius()) {
                //System.out.println("Predator ate a boid!");  //track when the predator eats the boid.
                return true;  //collision detected, predator eats the boid.
            }
        }
