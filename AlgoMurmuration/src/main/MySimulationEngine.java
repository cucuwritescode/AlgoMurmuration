...
public class MySimulationEngine {
 ...
 // Method to update boid speed
    public void updateBoidSpeed(int speed) {
        ...
        for (MyBoid b : boids) {
            b.setSpeed(boidSpeed);  // Update the speed of each boid
        }
    }
    ...
    public void updatePredatorSpeed(int speed) {
        ...
        for (Predator p : predators) {
            if (p instanceof MyHawk) {
                ((MyHawk) p).setSpeed(predatorSpeed);  // Update the predator speed
            }
        }
    }
    ...
    public void removePredator() {
        if (predators.size() > 0) {
            Predator predator = predators.remove(predators.size() - 1);  // Remove last predator
            predator.unDraw();  // Optionally, undraw the removed predator
    ...
    private void attachMouseHandlers() {
        // Mouse Handlers for obstacles here ...
        ...
        canvas.addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                if (e.getButton() == MouseEvent.BUTTON1) {
                    ...
                    synchronized (obstacles) {
                        obstacles.remove(obstacles.size() - 1);  // Remove the current mouse obstacle (if any)
                    }
                }
            }
    ...
    private void update(int deltaTime) {
        ...
        for (Predator p : predators) {
            if (p instanceof MyHawk) {
                MyHawk predator = (MyHawk) p;

                // Check if there are boids and predators to hunt/eat
                if (boids.size() > 0 && predators.size() > 0) {
                    predator.hunt(new ArrayList<>(boids), deltaTime);  // Predator hunts boids

                    // Check for collision between predator and boids
                    for (MyBoid b : boids) {
                        if (predator.collisionCheck(b)) {
                            boidsToRemove.add(b);  // Mark boid for removal
                            System.out.println("Boid eaten by predator!"); // Print once per removal
                        }
                    }
                }

                // Remove boids that were "eaten" by the predator after the loop
                for (MyBoid b : boidsToRemove) {
                    boids.remove(b); // Remove the boid
                }

                predator.avoidObstacles(obstacles);  // Avoid obstacles
                predator.wrapPosition(canvasWidth, canvasHeight);  // Handle wrapping
                predator.update(deltaTime);  // Update predator's movement
            }
        }
    }
